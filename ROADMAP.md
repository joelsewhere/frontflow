...
- Server-side submissions pagination — the form's Submissions tab paginates client-side at 25 rows per page. The submissions endpoint still returns the full list; the client just slices. Fine until a form has thousands of submissions, at which point the initial fetch and React rendering become the bottleneck. Closing this needs API params (`?limit=&offset=` or cursor-based) on `GET /forms/{id}/submissions`, plus a paginated query hook. The page-size knob — currently fixed at 25, no UI to change it — would naturally come along.
