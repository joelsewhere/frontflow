...
import {
  buildFilterMask,
  latestFilterDirectiveFor,
} from "./dashboardDirectives";
import {
...
export interface DashboardBlockProps {
  ...
  /** This rendering's own id, so a directive naming a panel reaches
   *  only the one it named. A workspace may show one dashboard twice —
   *  one panel following the submission, another left alone. */
  panelId?: string | null;
...
export function DashboardEmbed({
  ...
  const pendingFilters = latestFilterDirectiveFor(
    submission.data?.tasks,
    name,
    panelId,
  );
